...
\begin{document}
  ...
  \begin{enumerate}
    \item Convert the model's ARIMA equation from backshift notation to the equation where $y_t$ is the only term on the left side of the equation
    \item Replace $t$ with $T+S$ where $S$ is the number of future time steps
    \item Replace future observations with their predicted forecasts and future errors to zero
  \end{enumerate}
  Let's try this with an example using an ARIMA(2,1,2) model as an example. 
