...
\begin{document}
  ...
  \section{Non-seasonal Forecasting Models}
    There are many types of forecasting models but this paper will look two types. ARIMA (Auto-Regressive Integrated Moving Average) and Exponential Smoothing models. 
  ...
  \begin{equation*}
    Y_t = (y_t - y_{t-1}) - (y_{t-1} - y_{t-2}) = (y_t - 2By_t + B^2y_t) = (1 - B)^2y_t.
  \end{equation*}
  ...
  \begin{equation*}
    (1 - \phi_1B - \phi_2B^2)(1-B)y_t = \mu + (1 + \sigma_1B + \sigma_2B^2)\varepsilon_t.
  \end{equation*}
  Using step 1, we expand and group:
  \begin{align*}
    (1 - B - \phi_1B + \phi_1B^2 - \phi_2B^2 + \phi_2B^3)y_t &= \mu + (1 + \sigma_1B + \sigma_2B^2)\varepsilon_t\\
    [1 - (1 + \phi_1)B - (\phi_2 - \phi_1)B^2 + \phi_2B^3]y_t &= \mu + (1 + \sigma_1B + \sigma_2B^2)\varepsilon_t.
  \end{align*}

  With second step, we convert giving
  \begin{equation*}
    y_t - (1 + \phi_1)y_{t-1} - (\phi_2 - \phi_1)y_{t-2} + \phi_2y_{t-3} = \mu + \varepsilon_t + \sigma_1\varepsilon_{t-1} + \sigma_2\varepsilon_{t-2},
  \end{equation*}
  and moving terms we obtain
  \begin{equation*}
        y_t = \mu + (1 + \phi_1)y_{t-1} + (\phi_2 - \phi_1)y_{t-2} - \phi_2y_{t-3} + \varepsilon_t + \sigma_1\varepsilon_{t-1} + \sigma_2\varepsilon_{t-2}.
  \end{equation*}

  Third step is to replace $t$ with $T+1$:
  \begin{equation*}
    y_{T+1} = \mu + (1 + \phi_1)y_{T} + (\phi_2 - \phi_1)y_{T-1} - \phi_2y_{T-2} + \varepsilon_{T} + \sigma_1\varepsilon_{T-1} + \sigma_2\varepsilon_{T-2}.
  \end{equation*}
  ...
  \begin{equation*}
    y_{T+1} = \mu + (1 + \phi_1)y_{T} + (\phi_2 - \phi_1)y_{T-1} - \phi_2y_{T-2} + \sigma_1\varepsilon_{T-1} + \sigma_2\varepsilon_{T-2}.
  \end{equation*}
  To predict further into the future say at time $T+2$, then just repeat step 3 which produces
  \begin{equation*}
    y_{T+2} = \mu + (1 + \phi_1)y_{T+1} + (\phi_2 - \phi_1)y_{T} - \phi_2y_{T-1} + \sigma_2\varepsilon_{T-1}.
  \end{equation*}
  Observe that there should be a $\sigma_1\varepsilon_{T}$ term but since we set the time series value at $T$ to be equal to the predicted value, this error term evaluates to 0. Hence, if you predict $S > q$ future time steps, then all MA terms disappear. 
  ...
  Clearly, $ARIMA(0,1,0)$ is the better performer so $d=1$ is a good choice. Using this method can be preferred over the trial and error method with the ADF test if we only care about differencing our data.
  ...
  The slow decrease in the ACF signifies and p-value $>=0.05$ indicates that we need more orders of differencing. This time we will try include a non-seasonal difference.

  This looks a lot better. With a p-value $<0.05$ we can be happy with our orders of differencing being $d=1$ and $D=1$. Note that the order in which you difference the series, either seasonal or non-seasonal first, does not make a difference to the final differenced series.
